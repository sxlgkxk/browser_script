(function () {
	...
	async function public_gist_get_async(gist_id, filename) {
		log = sessionStorage.getItem('ex_public_log_cache')
		if (!log || window.forceRefresh) {
			window.forceRefresh = false
			response = await axios.get("https://api.github.com/gists/" + gist_id)
			data = response.data
			content = data.files[filename].content
			sessionStorage.setItem('ex_public_log_cache', content)
			return content
		} else
			return log
	}
...
`)
	...
	function refreshHeatmap() {
		...
		public_gist_get_async(gist_id, filename).then((content) => {
